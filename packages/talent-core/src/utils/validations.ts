...
import type { Candidate, ValidationResult, Vacancy } from "../types/models.js";
...
export function validateCandidate(candidate: Candidate): ValidationResult {
  ...
  if (
    ...
    errors.push(
      `yearsOfExperience debe estar entre ${MIN_YEARS_OF_EXPERIENCE} y ${MAX_YEARS_OF_EXPERIENCE}`,
    );
  }
...
export function validateVacancy(vacancy: Vacancy): ValidationResult {
  ...
  if (
    ...
    errors.push(
      `minYearsExperience debe ser mayor o igual que ${MIN_YEARS_OF_EXPERIENCE}`,
    );
  }
